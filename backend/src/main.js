...
app.get("/get-sections", async (req, res) => {
    ...
    const sectionData = await Database.Query("SELECT type, time_limit, required, section_index, algorithm_id FROM `sections` WHERE level_id=? ORDER BY section_index ASC", [level]);
    try {
